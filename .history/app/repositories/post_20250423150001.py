...
class PostRepository:
    ...
    async def del_all_posts(self) -> bool:
        """
        Удаляет все посты из базы и из Google Sheets.
        :return: True, если хотя бы одна строка была удалена.
        """
        stmt = delete(Post)
        result = await self.session.execute(stmt)
        await self.session.commit()
    ...
    async def save_one(self, post: Post) -> int | None:
        """
        Сохраняет один пост в базу и добавляет в Google Sheets.
        :param post: Объект Post.
        :return: ID нового поста или None при ошибке.
        """
        try:
            await self.update_id_seq()
    ...
    async def get_all_posts_id(self) -> list[int]:
        """
        Получает список всех ID постов.
    ...
    async def get_post_by_id(self, id:int) -> list[Post]:
        """
        Получает посты по ID.
        :param id: ID поста.
        :return: Список объектов Post (обычно один элемент).
        """
