...
async def process_body (message: Message, state:FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        post_id = await repo.save_one(post)
if post_id is None:
    await message.answer("❌ Не удалось сохранить пост. Попробуйте позже.")
    return
        await state.clear()
        buttons = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="➕ Добавить еще пост", callback_data="create_post")],
