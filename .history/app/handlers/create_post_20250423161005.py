...
from app.services import create_post_service
...
async def ask_post_title(callback:CallbackQuery, state:FSMContext):
    """
    Обработчик кнопки "➕ Добавить пост".
    Запрашивает у пользователя заголовок нового поста и устанавливает состояние ожидания заголовка.
    """
...
async def process_post_id(message: Message, state: FSMContext):
    """
    Обрабатывает введённый пользователем заголовок поста.
    Сохраняет его во временное хранилище FSM и переводит в состояние ожидания текста поста.
    """
...
async def process_body(message: Message, state: FSMContext):
    """
    Обрабатывает текст статьи, завершает создание поста.
    Сохраняет пост в базу данных и в Google Таблицу.
    Показывает пользователю результат и кнопки навигации.
    """
    ...
    async with SessionLocal() as session:
        post_id = await create_post_service(session, user_id, title, body)
