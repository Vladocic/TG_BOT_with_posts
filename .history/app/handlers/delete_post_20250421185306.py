...
async def handle_delete_post(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
...
async def ask_post_id_to_delete(message:Message, state: FSMContext, session: AsyncSession ):
